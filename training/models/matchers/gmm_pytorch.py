...
class GaussianMixture(torch.nn.Module):
    ...
    def predict(self, x, probs=False):
        ...
        if probs:
            ...
            return torch.squeeze(p_k / (p_k.sum(1, keepdim=True))), self.mu
        else:
            return torch.squeeze(torch.max(weighted_log_prob, 1)[1].type(torch.LongTensor)), self.mu
    ...
    def _get_unique_center(self, x, n_centers):
        """
        初始化中心点，确保第一维不同。
        """
        selected_indices = []
        unique_values = set()
        time_s = 0
        while len(selected_indices) < n_centers:
            time_s += 1
            if (time_s % 100 == 0):
                print(time_s)
            idx = np.random.choice(x.shape[0])
            if x[idx, 0].item() not in unique_values:  # 检查第一维是否唯一
                unique_values.add(x[idx, 0].item())
    ...
    def get_kmeans_mu(self, x, n_centers, init_times=50, min_delta=1e-3):
        ...
        for i in range(init_times):
            tmp_center = self._get_unique_center(x, n_centers)
