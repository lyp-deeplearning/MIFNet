...
def training(rank, conf, output_dir, args):
    ...
    if args.overfit:
        ...
        val_loader = val_dataset.get_overfit_loader("train")
        
    else:
        train_loader = dataset.get_data_loader("train", distributed=args.distributed)
        val_loader = val_dataset.get_data_loader("train")
    if rank == 0:
    ...
    if load_direct:
        ...
        for i in range(9):
                ...
                checkpoint = {k.replace(*pattern): v for k, v in checkpoint.items()}
                pattern = f"log_assignment.{i}", f"matcher.log_assignment.{i}"
                checkpoint = {k.replace(*pattern): v for k, v in checkpoint.items()}
                pattern = f"token_confidence.{i}", f"matcher.token_confidence.{i}"
                checkpoint = {k.replace(*pattern): v for k, v in checkpoint.items()}
        
        keys_a = set(model.state_dict().keys())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_direct", action="store_true")
