...
sd_model_dir =  Path(__file__).resolve().parent.parent.parent / "diffusion_weight/stable-diffusion-2-1"
...
class SDFeaturizer:
    ...
    def __init__(self, sd_id=sd_model_dir, null_prompt=''):
...
class SDFeaturizer4Eval(SDFeaturizer):
    def __init__(self, sd_id=sd_model_dir, null_prompt='', cat_list=[]):
    ...
    def forward(self,
        ...
        if category in self.cat2prompt_embeds:
            ...
            prompt_embeds = self.null_prompt_embeds
        prompt_embeds = prompt_embeds.repeat(ensemble_size, 1, 1).cuda() #([8, 77, 1024]),没看懂这个prompt是什么
        ...
        unet_ft = unet_ft_all['up_ft'][up_ft_index] # ensem, c, h, w, ([8, 640, 96, 96])
        unet_ft = unet_ft.mean(0, keepdim=True) # 1,c,h,w
